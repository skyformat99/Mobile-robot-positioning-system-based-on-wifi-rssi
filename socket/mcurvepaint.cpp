...
void mCurvePaint::setPoint(double xx, double yy)
{
    x=(qreal)xx;
    y=(qreal)yy;
}

void mCurvePaint::setPointon(bool r)
{
    isPointon = r;
}

void mCurvePaint::setPointcolor(Qt::GlobalColor color)
{
...
void mCurvePaint::paint()
{
    ...
    //坐标系设置
        ...
        for(i=1 ; i <= 5 ; i++)
        {
            QRectF rect = QRectF(0,myTransform(-3*2,i*(height/5)+1).y(),
                                 x1,myTransform(0,i*(height/5)-5).y());
        ...
        if(isPointon)
        {
            painter->drawPoint(myTransform(x,y));
        }
