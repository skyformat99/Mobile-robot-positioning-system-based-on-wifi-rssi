     ...
     char **	argv)
{
  ...
  int count,i=0;
   ...
   /* 网络通信相关变量 */
    ...
    remote_addr.sin_addr.s_addr=inet_addr("192.168.0.145");//服务器IP地址  
...
//清空结构体数组缓存
 ...
 /*创建客户端套接字--IPv4协议，面向连接通信，TCP协议 */ 
    ...
    {  
      ...
      struct ifreq struIR;
      strncpy(struIR.ifr_name, "eth0", IFNAMSIZ);
  ...
  //  fcntl(client_sockfd,F_SETFL,flags |O_NONBLOCK);    //设置成阻塞模式；
	...
	while(1)
	{
		...
		if(print_scanning_info(skfd, dev,  data, (int *)&count) == 0)
		{
			...
			int ret = send(client_sockfd, socket_snd_buf, sizeof(socket_info)+8,0);
			if(ret < 0)
			{
				perror("write");  
				sendflag =0;
			...
			/* int ret = socket_send(client_sockfd, socket_snd_buf, sizeof(socket_info)+8);
			  if(ret == sizeof(socket_info)+8)
			  {
				printf("send ok!\n");
			  }
