...
{
	int st =*(int*)arg;
//	char data[SEND_BUFF_SIZE];
	//char socket_snd_buf[SEND_BUFF_SIZE];
	int i=0;
	while(1)
	{
		usleep(1000*1000);
		if(thread_info.update == 1)
		{
		//	pthread_mutex_lock(&mutx);
			thread_info.update = 0;
		//	pthread_mutex_unlock(&mutx);
			
			//memset(socket_snd_buf, 0, sizeof(socket_snd_buf)); //清空缓存
			//memcpy(socket_snd_buf, &thread_info.socket_info, sizeof(thread_info.socket_info));  //拷贝数据到发送缓冲区中等待发送
			if(send(st, thread_info.socket_snd_buf, sizeof(thread_info.socket_snd_buf),0)<0)
			{
				perror("write");  
	         		 pthread_exit(NULL);  
	         		printf("send failed thread exit!\n");
	         		break;
			}
			//i++;
			//if(i == 1)
			//{
			//	printf("thread in haha!\n");
			//	i=0;
			//}
		}
	}
...
//清空结构体数组缓存
  ...
  /*创建服务器端套接字--IPv4协议，面向连接通信，TCP协议*/  
   ...
   while(1)
	...
	if(thread_info.update == 0)
	{
		printf("in\n");
		print_scanning_info(skfd, dev,  data, (int *)&count);
		//sleep(1);
		// i++;
	//	if(i == 1) 
		{
		...
		printf("main:%d\n",i);
		  /* 组包 */
		memset(&socket_info, 0, sizeof(socket_info));//清空结构体数据
		printf("main:%d\n",++i);
		memcpy(socket_info.head, head, 8);//添加包头
		printf("main:%d\n",++i);
		memcpy(socket_info.buff, data, sizeof(data)); //填充数据
		printf("main:%d\n",++i);
		socket_info.count = count;//个数
		memset(thread_info.socket_snd_buf, 0, sizeof(thread_info.socket_snd_buf)); //清空缓存
		printf("main:%d\n",++i);
		memcpy(thread_info.socket_snd_buf, &socket_info, sizeof(socket_info));  //拷贝数据到发送缓冲区中等待发送
		printf("main:%d\n",++i);
		//pthread_mutex_lock(&mutx);
		printf("main:%d\n",++i);
		thread_info.update = 1;
		//pthread_mutex_unlock(&mutx);
		printf("main:%d\n",++i);
		i=0;
	}
	...
	//  memcpy(socket_snd_buf, &socket_info, sizeof(socket_info));  //拷贝数据到发送缓冲区中等待发送

      //  if(send(client_sockfd,data,sizeof(data),0)<0)  
   //     {  
       //     perror("write");  
      //      return 1;  
     //   }  
