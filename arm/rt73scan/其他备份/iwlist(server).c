...
//清空结构体数组缓存
  ...
  /*创建服务器端套接字--IPv4协议，面向连接通信，TCP协议*/  
    ...
    int flags  = fcntl(server_sockfd,F_GETFL,0);
      ...
      fcntl(server_sockfd,F_SETFL,flags |O_NONBLOCK);    //设置成阻塞模式；
char flag=1,sendflag=0;
char headd[4] = {0x1a, 0x2a, 0x3a, 0x4a};
char tial[4] = {0x1f, 0x2f, 0x3f, 0x4f};
...
//	  if(flag == 1)
  	{
		  printf("in printfinfo now!\n");
		if(print_scanning_info(skfd, dev,  data, (int *)&count) == 0)
		{
			sendflag = 1;
			//print_freq_info(skfd,dev,NULL,0);
			//  memcpy(&sen_buf, data, sizeof(data));
			//  for(i=0; i<count; i++)
			  {
			 //     printf("data[%d].data = %s, data[%d].level %d\n", i, sen_buf[i].data, i, sen_buf[i].level);
			  }
			  ...
			  //添加包头
			  memcpy(socket_info.head, "essid", 8);
			  //填充数据
			  memcpy(socket_info.buff, data, sizeof(data));
			  socket_info.count = count;
			  //清空缓存
			  memset(socket_snd_buf, 0, sizeof(socket_snd_buf));
			 // memcpy(socket_snd_buf, headd, 4);
			...
			//  socket_snd_buf[5+sizeof(socket_info)+1] = 0x1f;
			  socket_snd_buf[4+sizeof(socket_info)+1] = 0x1f;
			  socket_snd_buf[4+sizeof(socket_info)+2] = 0x2f;
			  socket_snd_buf[4+sizeof(socket_info)+3] = 0x3f;
			  socket_snd_buf[4+sizeof(socket_info)+4] = 0x4f;
			//  memcpy(socket_snd_buf+5+sizeof(socket_info)+1, tial, 4);

			int ret = send(client_sockfd, socket_snd_buf, sizeof(socket_info)+9,0);
			if(ret < 0)
			{
				perror("write");  
				sendflag =0;
			...
			else if(ret >= 0 && ret < sizeof(socket_snd_buf)){
				flag = 0;
				sendflag = 1;
...
/*  int ret = socket_send(client_sockfd, socket_snd_buf, 8);
			  if(ret == 8)
			  {
				printf("send ok!\n");
			  }
...
//	if(sendflag == 1)
	...
	{
		int ret = send(client_sockfd, socket_snd_buf, sizeof(socket_snd_buf),0);
		if(ret < 0)
		{
			perror("write");  
			sendflag =0;
...
*/
	/*  解析包  
	  //清空缓存
	  memset(&socket_info1, 0, sizeof(socket_info1));
	  //拷贝数据到发送缓冲区中等待发送
	  memcpy(&socket_info1, socket_snd_buf, sizeof(socket_info1));*/

	  /* 打印数据 
	  memcpy(&sen_buf, socket_info1.buff, sizeof(socket_info1.buff));
	  for(i=0; i<count; i++)
	  {
	      printf("data[%d].data = %s, data[%d].level %d\n", i, sen_buf[i].data, i, sen_buf[i].level);
	  }
